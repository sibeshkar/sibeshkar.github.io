...
1. He was building on some previous work, but that a human with no tooling/computation access could speedrun a 5000x compression in a lifetime is astonishing. A 21st century analogue would be if Linus Torvalds hand-wrote the final, complete, version of Linux on his first try without access to a compiler - a complete version which would require no patches since inception. 
2. It's bizarre that an unchanging rule-based generative order even exists for an evolving spoken language, that was lying there waiting to be discovered. It's as bizarre as counting the seed spirals of a sunflower and finding that they always follow some predetermined recursive Fibonacci sequence, or appear in fixed golden ratios of phi (1.618...). This is a discussion for another day.
